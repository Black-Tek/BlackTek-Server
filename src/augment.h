...
public:
	...
	bool unserialize(PropStream& propReadStream) {
		...
		std::cout << "INFO: Attack modifier count: " << attackModifierCount << std::endl;
		...
		std::cout << "INFO: Defense modifier count: " << defenseModifierCount << std::endl;
...
inline void Augment::addModifier(std::shared_ptr<DamageModifier>& mod) {
	if (mod->getStance() == ATTACK_MOD) {
		...
		std::cout << "Size of defense modifiers " << m_defense_modifiers.size() << " \n";
	}
}
