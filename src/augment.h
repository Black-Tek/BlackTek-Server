...
public:
	...
	void serialize(PropWriteStream& propWriteStream) const {
		...
		propWriteStream.writeString(std::string{ m_name.data()});
		propWriteStream.writeString(std::string{ m_description.data() });
	...
	bool unserialize(PropStream& propReadStream) {
		...
		if (!successDesc) return false;
		m_description = description;

		// Deserialize m_attack_modifiers
		uint32_t attackModifierCount;
		if (!propReadStream.read<uint32_t>(attackModifierCount)) return false;  // Read the number of attack modifiers

		m_attack_modifiers.clear();  // Clear the existing modifiers before deserialization
		for (uint32_t i = 0; i < attackModifierCount; ++i) {
			auto modifier = std::make_shared<DamageModifier>();  // Create a new DamageModifier instance
			if (!modifier->unserialize(propReadStream)) return false;  // Unserialize each modifier
			m_attack_modifiers.push_back(modifier);  // Add the modifier to the vector
		}

		// Deserialize m_defense_modifiers
		uint32_t defenseModifierCount;
		if (!propReadStream.read<uint32_t>(defenseModifierCount)) return false;  // Read the number of defense modifiers

		m_defense_modifiers.clear();  // Clear the existing modifiers before deserialization
		for (uint32_t i = 0; i < defenseModifierCount; ++i) {
			auto modifier = std::make_shared<DamageModifier>();  // Create a new DamageModifier instance
			if (!modifier->unserialize(propReadStream)) return false;  // Unserialize each modifier
			m_defense_modifiers.push_back(modifier);  // Add the modifier to the vector
		}
