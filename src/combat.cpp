...
// to-do: use doCombat and set origin to augment.
...
{	
	...
	if (casterPlayer && target) {
		...
		auto targetType = (targetPlayer.has_value()) ? CREATURETYPE_PLAYER : casterPlayer.value()->getCreatureType(*targetMonster.value()->getMonster());
			...
			if (!attackModData.empty() && params.origin != ORIGIN_AUGMENT) {
				...
				if (piercingPercentTotal || piercingFlatTotal) {
					...
					if (piercingPercentTotal) {
						...
						piercingDamage = (piercingPercentTotal <= 100) ? damage.primary.value * (percentValue / 100.0) : damage.primary.value;
					}
					...
					if (piercingDamage) {
						...
						auto trueDamage = std::abs(piercingDamage);
						auto difference = std::abs(damage.primary.value) - trueDamage;
