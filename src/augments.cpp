                    ...
                    if (auto mod_list = modifier_data.as_array()) {
                        mod_list->for_each([augment, name](auto&& prop) {
                            if (prop.is_table()) {
                                ...
                                std::string_view creatureType =     table["creatureType"].value_or("none");
                                ...
                                if (ParseStance(stance) == ATTACK_MOD) {

                                    std::cout << "[ Augments ] Parsing Attack Augment : " << name << " \n";
                                    std::cout << ":: Mod Type = " << modType << " and parses to : " << ParseAttackModifier << std::endl;
                                    std::cout << ":: Amount = " << amount << std::endl;
                                    std::cout << ":: Factor = " << factor << " and casts to : " << std::bit_cast<ModFactor>(factor) << std::endl;
                                    std::cout << ":: Chance = " << chance << std::endl;
                                    std::cout << ":: Damage Type = " << damageType << " and parses to : " << ParseDamage(damageType) << std::endl;
                                    std::cout << ":: Origin Type = " << originType << " and parses to : " << ParseOrigin(originType) << std::endl;
                                    std::cout << ":: Creature Type = " << creatureType << " and parses to : " << ParseCreatureType(creatureType) << std::endl;
                                    std::cout << ":: Race Type = " << race << " and parses to : " << ParseRaceType(race) << std::endl;
                                    ...
                                    if (creatureName != "none") {
                                        std::cout << "Found valid creature name " << creatureName.data() << "\n";
                                        damage_modifier->setCreatureName(creatureName);
                                    }

                                    augment->addModifier(damage_modifier);

                                } else if (ParseStance(stance) == DEFENSE_MOD) {

                                    std::cout << "[ Augments ] Parsing Defense Augment : " << name << " \n";
                                    std::cout << ":: Mod Type = " << modType << " and parses to : " << ParseAttackModifier << std::endl;
                                    std::cout << ":: Amount = " << amount << std::endl;
                                    std::cout << ":: Factor = " << factor << " and casts to : " << std::bit_cast<ModFactor>(factor) << std::endl;
                                    std::cout << ":: Chance = " << chance << std::endl;
...
const ModifierStance Augments::ParseStance(std::string_view stanceName) noexcept
{
...
{
    std::cout << "Augment name passed : " << augName << "\n";
    auto it = global_augments.find(augName.data());
    if (it != global_augments.end()) {
        std::cout << "Found the augment! \n";
