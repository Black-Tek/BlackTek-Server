...
local function InteractiveConsole(env)
    return coroutine.wrap(function(inp, out, err)
        ...
        while true do
            ...
            -- FIX: In Lua 5.2+, 'load' is used instead of 'loadstring' + 'setfenv'
            -- The 4th argument applies our 'sandbox' environment automatically
            local func, e = load(table.concat(chunks, ' '), 'console', 't', sandbox)
            if func then            
