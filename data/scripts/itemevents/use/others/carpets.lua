...
local carpet = ItemEvent()

carpet.onUse = function(player, item, fromPosition, target, toPosition, isHotkey)
	local carpetId = transformID[item.itemid]
	...
	local carpetStack = 0
	for _, id in pairs(transformID) do
		carpetStack = carpetStack + tile:getItemCountById(id)
	end
