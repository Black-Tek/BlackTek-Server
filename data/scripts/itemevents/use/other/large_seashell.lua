local largeSeashell = ItemEvent()

largeSeashell.onUse = function(player, item, fromPosition, target, toPosition, isHotkey)
	if player:getStorageValue(PlayerStorageKeys.delayLargeSeaShell) <= os.time() then
		local chance = math.random(100)
		local msg = ""
		if chance <= 16 then
			doTargetCombat(0, player, Combat.DamageType.Physical, -200, -200, CONST_ME_NONE)
