...
function gameStorePurchase(player, offer)
	...
	for i = 1, #offers do
		if offers[i].title == offer.title and offers[i].price == offer.price then
			...
			db.query("UPDATE `web_accounts` set `shop_points` = `shop_points` - " .. offers[i].price .. " WHERE `account_id` = " .. aid)
			db.asyncQuery(
...
-- 	local points = 0
-- 	local resultId = db.storeQuery("SELECT `shop_points` FROM `web_accounts` WHERE `account_id` = " .. player:getAccountId())
-- 	if resultId ~= false then
-- 		points = result.getDataInt(resultId, "shop_points")
-- 		result.free(resultId)
-- 	end
-- 	return points
-- end
